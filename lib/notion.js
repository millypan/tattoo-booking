import {
  ...
  addDaysToDateString,
  taipeiTomorrowStartISO,
  isTomorrowOrLaterInTaipei,
} from "./slotFormat.mjs";
import {
  ...
  scheduleInterval,
  scheduleCategoryAfterPayment,
  scheduleTypeFromCategory,
} from "./scheduleConflict.mjs";
...
function minimalScheduleType(category) {
  return scheduleTypeFromCategory(category);
}

// Notion 的公開時段只有開始時間，沒有結束時間；未填明確 end 時，依目前排程習慣
// 以諮詢 2 小時、刺青 4 小時估算占用範圍。若米粒在日期欄填了 end，則以 end 為準。
// 只自動關閉「開放」空檔；保留中／已預約不靜默取消，改回傳 conflicts 供日誌追查。
async function closeOverlappingOpenSlots({ bookingId, start, end, category }) {
  const type = scheduleTypeFromCategory(category);
  const bookingInterval = scheduleInterval({ start, end, type });
  if (!bookingInterval) return { closed: [], conflicts: [] };

  const date = taipeiTodayDateString(start);
  const nextDate = addDaysToDateString(date, 1);
  const slots = await notionQueryAll(`/databases/${requireEnv("SLOTS_DB")}/query`, {
    filter: {
      and: [
        { property: "日期時間", date: { on_or_after: `${date}T00:00:00+08:00` } },
        { property: "日期時間", date: { before: `${nextDate}T00:00:00+08:00` } },
  ...
  for (const slot of slots) {
    if (slot.properties?.["關聯預約"]?.relation?.some((relation) => relation.id === bookingId)) continue;
    const slotStart = slotStartISO(slot);
    const slotType = slot.properties?.["類型"]?.select?.name;
    const slotInterval = scheduleInterval({ start: slotStart, type: slotType });
    if (!intervalsOverlap(bookingInterval, slotInterval)) continue;

    const status = slot.properties?.["狀態"]?.select?.name;
    if (status === "開放") {
      await closeSlot(slot.id);
      closed.push(slot.id);
    } else {
      conflicts.push({ id: slot.id, status, label: slotDisplayLabel(slot) });
    }
  }
...
async function syncMinimalSchedule(bookingId, start, category, existingScheduleId = null, scheduleStatus = "預定") {
  const type = minimalScheduleType(category);
  if (!bookingId || !start || !type) return null;
  const properties = {
    "排程": { title: [{ text: { content: minimalScheduleTitle(taipeiTimeLabel(start), type, scheduleStatus) } }] },
    "時間": { date: { start } },
    "類型": { select: { name: type } },
    "狀態": { select: { name: scheduleStatus } },
    "關聯預約": { relation: [{ id: bookingId }] },
    "同步識別": { rich_text: [{ text: { content: `booking:${bookingId.replaceAll("-", "")}` } }] },
  };
  if (existingScheduleId) {
    const updated = await notion(`/pages/${existingScheduleId}`, { archived: false, properties }, "PATCH");
    await convergeMinimalSchedules(bookingId, type);
    return updated;
  }
  ...
  // Notion 沒有唯一索引；首頁、Webhook 與排程可能在同一瞬間都通過「尚未存在」檢查。
  // 建立後再以固定排序收斂，確保併發時所有執行者都保留同一筆、封存其餘重複資料。
  await convergeMinimalSchedules(bookingId, type);
  return created;
}
...
async function retainCancelledMinimalSchedules(booking) {
  const scheduleStatus = cancellationScheduleStatus(
    booking.properties?.["取消類型"]?.select?.name
  );
  const existingRelations = booking.properties?.["極簡排程"]?.relation || [];
  const retainedTypes = new Set();

  for (const relation of existingRelations) {
    const schedule = await notion(`/pages/${relation.id}`, null, "GET");
    const start = schedule.properties?.["時間"]?.date?.start;
    const type = schedule.properties?.["類型"]?.select?.name;
    if (!start || !type) continue;
    retainedTypes.add(type);
    await syncMinimalSchedule(booking.id, start, type, relation.id, scheduleStatus);
  }

  const workStart = booking.properties?.["工作行程時間"]?.date?.start;
  const workCategory = booking.properties?.["行程分類"]?.select?.name;
  const workType = minimalScheduleType(workCategory);
  if (workStart && workType && !retainedTypes.has(workType)) {
    const existing = await findMinimalSchedule(booking.id, workType);
    await syncMinimalSchedule(booking.id, workStart, workCategory, existing?.id || null, scheduleStatus);
    retainedTypes.add(workType);
  }

  const tattooStart = booking.properties?.["刺青時間"]?.date?.start;
  if (tattooStart && !retainedTypes.has("刺青")) {
    const existing = await findMinimalSchedule(booking.id, "刺青");
...
async function findMinimalSchedule(bookingId, type) {
  const schedules = await findMinimalSchedules(bookingId, type);
  return schedules[0] || null;
}

async function findMinimalSchedules(bookingId, type) {
  return notionQueryAll(`/databases/${await getMinimalScheduleDbId()}/query`, {
    filter: {
      and: [
        {
          or: [
            { property: "同步識別", rich_text: { equals: `booking:${bookingId.replaceAll("-", "")}` } },
...
async function convergeMinimalSchedules(bookingId, type) {
  const schedules = await findMinimalSchedules(bookingId, type);
  const canonical = schedules[0] || null;
  for (const duplicate of schedules.slice(1)) {
    await notion(`/pages/${duplicate.id}`, { archived: true }, "PATCH");
  }
  return canonical;
}

// Notion Webhook 的單筆即時同步入口。Webhook 只提供變動頁面的 id，這裡重新讀取
// 最新欄位，確認它真的屬於預約訂單資料庫後，才建立或更新對應的極簡排程。
// GitHub 排程仍保留作補漏，但日常不再依賴它何時被排到。
export async function syncBookingMinimalSchedules(bookingId) {
  if (!bookingId) return { ignored: "missing_page_id" };

  const booking = await notion(`/pages/${bookingId}`, null, "GET");
  const parentDatabaseId = booking.parent?.database_id?.replaceAll("-", "");
  const bookingsDatabaseId = requireEnv("BOOKINGS_DB").replaceAll("-", "");
  if (parentDatabaseId !== bookingsDatabaseId) return { ignored: "not_a_booking" };

  const status = booking.properties?.["狀態"]?.select?.name;
  if (booking.archived || booking.in_trash) {
    await archiveMinimalSchedules(booking);
    return { archived: true };
  }
  if (status === "取消") {
    const retainedStatus = await retainCancelledMinimalSchedules(booking);
    return { cancelled: true, retainedStatus };
  }

  const start = booking.properties?.["工作行程時間"]?.date?.start;
  const end = booking.properties?.["工作行程時間"]?.date?.end;
  const category = booking.properties?.["行程分類"]?.select?.name;
  const type = minimalScheduleType(category);
  let workSchedule = null;
  let slotConflicts = { closed: [], conflicts: [] };
  const tattooStart = booking.properties?.["刺青時間"]?.date?.start;
  const separateTattoo = type === "刺青" && tattooStart
    && new Date(tattooStart).getTime() !== new Date(start).getTime();
  if (start && type && !separateTattoo) {
    slotConflicts = await closeOverlappingOpenSlots({ bookingId: booking.id, start, end, category });
    const existing = await findMinimalSchedule(booking.id, type);
    workSchedule = await syncMinimalSchedule(booking.id, start, category, existing?.id || null);
  }

  // 刺青時間另有已預約時段同步邏輯；沿用既有函式，確保手動改刺青日期時
  // 時段資料庫與極簡排程會一起更新。
  const tattooTimes = await syncTattooTimes(booking);
  return { workSchedule: Boolean(workSchedule), slotConflicts, tattooTimes };
}

// 手動在 Notion 建立完整預約時，Notion 不會主動呼叫網站；公開首頁下次載入時，
// 只替尚未有極簡排程的有效案件補建一筆。已存在者不反覆更新，避免首頁產生大量 API 呼叫。
export async function syncMissingMinimalSchedules({ reconcile = false } = {}) {
    ...
    filter: {
      and: [
        ...
        reconcile ? { or: [
          ...
          { property: "工作行程時間", date: { on_or_after: taipeiTodayStartISO() } },
        ] } : { property: "極簡排程", relation: { is_empty: true } },
      ],
    },
  });
  ...
  for (const booking of bookings) {
    ...
    const category = booking.properties?.["行程分類"]?.select?.name;
    const type = minimalScheduleType(category);
    const existing = type ? await findMinimalSchedule(booking.id, type) : null;
    if (!start || !type) continue;
    // 人工取消的行程不因備援核對而重新啟用。
    if (existing && ["已取消", "當日未到", "遲到取消"].includes(existing.properties?.["狀態"]?.select?.name)) continue;
    const tattooStart = booking.properties?.["刺青時間"]?.date?.start;
    // 工作時間可能保留前段諮詢，分類已改成刺青；不能拿它覆蓋後段刺青。
    if (type === "刺青" && tattooStart && new Date(tattooStart).getTime() !== new Date(start).getTime()) continue;
    const existingStart = existing?.properties?.["時間"]?.date?.start;
    if (existingStart && new Date(existingStart).getTime() === new Date(start).getTime()) continue;
    await closeOverlappingOpenSlots({ bookingId: booking.id, start, end, category });
    await syncMinimalSchedule(booking.id, start, category, existing?.id || null);
    synced += 1;
  }
  return synced;
}

// 後臺健康檢查：只回傳缺漏筆數，不帶客戶姓名或其他個資。
// 工作行程與刺青時間分開檢查，避免「已有諮詢排程」掩蓋「刺青排程仍缺少」。
export async function getMinimalSyncHealth() {
  const missingWorkBookings = await notionQueryAll(`/databases/${requireEnv("BOOKINGS_DB")}/query`, {
    filter: {
      and: [
        { property: "工作行程時間", date: { is_not_empty: true } },
        { property: "狀態", select: { does_not_equal: "取消" } },
  ...
  const tattooBookings = await notionQueryAll(`/databases/${requireEnv("BOOKINGS_DB")}/query`, {
    filter: {
      and: [
        { property: "類型", select: { equals: "客製圖" } },
        { property: "刺青時間", date: { is_not_empty: true } },
        { property: "狀態", select: { does_not_equal: "取消" } },
        { property: "狀態", select: { does_not_equal: "完成" } },
      ],
    },
  ...
  let missingTattoo = 0;
  for (const booking of tattooBookings) {
    if (!(await findMinimalSchedule(booking.id, "刺青"))) missingTattoo += 1;
  }

  return {
    pendingWork: missingWorkBookings.length,
    pendingTattoo: missingTattoo,
    pending: missingWorkBookings.length + missingTattoo,
    checkedAt: new Date().toISOString(),
  };
}

// 客製訂單以諮詢為主體；米粒只需在同一列填「刺青時間」。每 5 分鐘把該時間
// 同步到已預約時段與極簡排程。修改時間時更新同一筆，不要求另開行程資料庫。
export async function syncTattooTimes(booking = null) {
  const eligible = booking && !booking.archived && !booking.in_trash
    && booking.properties?.["類型"]?.select?.name === "客製圖"
    && booking.properties?.["刺青時間"]?.date?.start
    && !["取消", "完成"].includes(booking.properties?.["狀態"]?.select?.name);
  // Webhook 只同步收到事件的訂單；排程呼叫不帶參數時才做全量補漏。
  const bookings = booking ? (eligible ? [booking] : []) : await notionQueryAll(`/databases/${requireEnv("BOOKINGS_DB")}/query`, {
    filter: {
      and: [
        { property: "類型", select: { equals: "客製圖" } },
        { property: "刺青時間", date: { is_not_empty: true } },
  ...
  let minimalUpdated = 0;
  for (const booking of bookings) {
    const start = booking.properties?.["刺青時間"]?.date?.start;
    const end = booking.properties?.["刺青時間"]?.date?.end;
    if (!start) continue;
    await closeOverlappingOpenSlots({
      bookingId: booking.id,
      start,
      end,
      category: "刺青｜已付定金",
    });
    const existingSlots = await notionQueryAll(`/databases/${requireEnv("SLOTS_DB")}/query`, {
      filter: {
        and: [
          { property: "關聯預約", relation: { contains: booking.id } },
          { property: "類型", select: { equals: "刺青" } },
          { property: "狀態", select: { does_not_equal: "關閉" } },
        ],
      },
    });
    const slotProperties = {
      "時段名稱": { title: [{ text: { content: formatSlotLabel(start) } }] },
      "日期時間": { date: { start } },
      "時段": { select: { name: taipeiTimeLabel(start) } },
      "類型": { select: { name: "刺青" } },
      "狀態": { select: { name: "已預約" } },
      "關聯預約": { relation: [{ id: booking.id }] },
    };
    if (existingSlots[0]) {
      await notion(`/pages/${existingSlots[0].id}`, { properties: slotProperties }, "PATCH");
      slotsUpdated += 1;
    } else {
      await notion(`/pages`, { parent: { database_id: requireEnv("SLOTS_DB") }, properties: slotProperties });
      slotsCreated += 1;
    }

    const tattooMinimal = await findMinimalSchedule(booking.id, "刺青");
    await syncMinimalSchedule(booking.id, start, "刺青｜已付定金", tattooMinimal?.id || null);
    if (tattooMinimal) {
      minimalUpdated += 1;
    } else {
      minimalCreated += 1;
  ...
  return { slotsCreated, slotsUpdated, minimalCreated, minimalUpdated };
}
...
// 米粒在 Notion 將訂單改為「取消」後，保留極簡行事曆紀錄，並安全釋放仍由該訂單占用的時段，
...
async function releaseCancelledSlots() {
  ...
  for (const booking of cancelled) {
    await retainCancelledMinimalSchedules(booking);
    ...
    if (slotId) {
      ...
      const slotStatus = slot.properties?.["狀態"]?.select?.name;
      if (["保留中", "已預約"].includes(slotStatus) && linkedBooking === booking.id) {
  ...
  const results = await notionQueryAll(`/databases/${requireEnv("SLOTS_DB")}/query`, {
    filter: {
      and: [
        ...
        { property: "類型", select: { equals: type } },
        { property: "日期時間", date: { on_or_after: taipeiTomorrowStartISO() } },
      ],
    },
...
export async function reserveCustomConsultation({ token, slotId }) {
  ...
  const isPubliclyBookable = start && isTomorrowOrLaterInTaipei(start);
  const expectedType = booking.type === "認領圖" ? "刺青" : "諮詢";
  if (p["類型"]?.select?.name !== expectedType || p["狀態"]?.select?.name !== "開放" || !isPubliclyBookable) {
...
export async function batchUpdateAdminSlots({ updates }) {
  ...
  for (const item of updates) {
    try {
      ...
      // 米粒可能已先用「開時段」建立好新時間；此時目的時間已存在，
      // 批量修改的正確結果是關閉舊時間，保留已建立的新時間。
      if (error.message === "這個日期時間已經有其他時段") {
        await closeSlot(item.id);
...
export async function confirmAdminPayment({ bookingId, amount, method, nature, lastFive, requestId }) {
  ...
  if (duplicate.length) {
    const sync = await syncBookingMinimalSchedules(bookingId);
    return { ok: true, duplicate: true, sync };
  }
  ...
  if (nextStatus && nextStatus !== booking.status) {
    // 客製案件的原工作行程通常是諮詢；後續收到定金時，刺青另由「刺青時間」建立排程，
    // 不可把原諮詢行程改名成刺青。只有認領圖本身的工作行程才跟著切換成刺青已付定金。
    const category = scheduleCategoryAfterPayment(nature, booking.type);
  ...
  // 收款成功即直接完成該筆的工作行程＋刺青排程，不等待 Notion Webhook。
  const sync = await syncBookingMinimalSchedules(bookingId);
  return { ok: true, duplicate: false, status: nextStatus, sync };
}
...
export async function createClaimBooking({ name, phone, spot, workId, slotId, sizeChoice, tryOnUploadId, coverPhotoUploadId, applicationType }) {
  ...
  if (slotId) {
    selectedSlot = await notion(`/pages/${slotId}`, null, "GET");
    const selectedStart = slotStartISO(selectedSlot);
    const selectedProperties = selectedSlot.properties;
    if (
      selectedProperties["類型"]?.select?.name !== "刺青" ||
      selectedProperties["狀態"]?.select?.name !== "開放" ||
      !selectedStart ||
      !isTomorrowOrLaterInTaipei(selectedStart)
    ) {
      throw new Error("SLOT_UNAVAILABLE");
    }
  }
  ...
  if (slotId) {
    const start = slotStartISO(selectedSlot);
