...
async function findMinimalSchedule(bookingId, type) {
  const schedules = await notionQueryAll(`/databases/${await getMinimalScheduleDbId()}/query`, {
    filter: {
      and: [
...
export async function syncTattooTimes() {
  ...
  for (const booking of bookings) {
    ...
    const tattooMinimal = await notionQueryAll(`/databases/${await getMinimalScheduleDbId()}/query`, {
      filter: {
        and: [
          { property: "關聯預約", relation: { contains: booking.id } },
          { property: "類型", select: { equals: "刺青" } },
    ...
    await syncMinimalSchedule(booking.id, start, "刺青｜已付定金", tattooMinimal[0]?.id || null);
    if (tattooMinimal[0]) {
      minimalUpdated += 1;
    } else {
      minimalCreated += 1;
