...
export async function getFutureSlots() {
  ...
  return results.map((page) => {
    ...
    const end = p["日期時間"]?.date?.end || null;
    ...
    return {
      ...
      displayEndTime: end?.includes("T") ? end.slice(11, 16) : null,
      type: p["類型"]?.select?.name || null,
      status: p["狀態"]?.select?.name || null,
    };
  });
...
export async function createSlots({ slots }) {
  ...
  for (let i = 0; i < slots.length; i++) {
    const { date, time, endTime, type } = slots[i];
    const iso = `${date}T${time}:00+08:00`;
    const endIso = endTime ? `${date}T${endTime}:00+08:00` : null;
    const ts = new Date(iso).getTime();
    const label = `${formatSlotLabel(iso)}–${endTime}`;
    ...
    try {
      await notion(`/pages`, {
        ...
        properties: {
          ...
          "日期時間": { date: { start: iso, ...(endIso ? { end: endIso } : {}) } },
          "類型": { select: { name: type } },
          "狀態": { select: { name: "開放" } },
        },
      });
...
export async function updateAdminSlot({ id, date, time, endTime, type }) {
  const iso = `${date}T${time}:00+08:00`;
  const endIso = endTime ? `${date}T${endTime}:00+08:00` : null;
  ...
  const updated = await notion(`/pages/${id}`, {
    properties: {
      "時段名稱": { title: [{ text: { content: `${formatSlotLabel(iso)}–${endTime}` } }] },
      "日期時間": { date: { start: iso, ...(endIso ? { end: endIso } : {}) } },
      "類型": { select: { name: type } },
    },
  }, "PATCH");
  ...
  for (const relation of slot.properties?.["關聯預約"]?.relation || []) {
    ...
    await notion(`/pages/${relation.id}`, {
      properties: {
        "工作行程時間": { date: { start: iso, ...(endIso ? { end: endIso } : {}) } },
...
export async function createClaimBooking({ name, spot, workId, slotId, sizeChoice, tryOnUploadId, coverPhotoUploadId, applicationType }) {
...
export async function createCustomBooking({ name, projectType, story, ref, spot, partPhotoUploadId, refPhotoUploadId }) {
  const customerId = await findOrCreateCustomer(name);
