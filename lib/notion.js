...
export const SETTINGS_DB = process.env.SETTINGS_DB || "3aed5c43-ea67-812f-89d9-dd4476edb61e";
...
export async function getOpenSlots(type) {
  const results = await notionQueryAll(`/databases/${SLOTS_DB}/query`, {
    filter: {
      and: [
...
export async function createSlots({ slots, type }) {
  const existingPages = await notionQueryAll(`/databases/${SLOTS_DB}/query`, {
    filter: {
      and: [
        { property: "日期時間", date: { on_or_after: taipeiTodayStartISO() } },
...
async function findSettingsRow(itemName) {
  const r = await notion(`/databases/${SETTINGS_DB}/query`, {
